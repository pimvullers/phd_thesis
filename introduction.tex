...
procedure\index{verification procedure} of the credential. It ensures that a
...
\index{authentication} becomes more privacy friendly. This latter process is
...
signatures}. Here the issuance involves creating a blind signature which
...
The use of zero-knowledge proofs\index{zero-knowledge proofs} allows a user to
...
\index{shared keys} across large batches of cards~\cite{Kugler2010}. The
...
protocols are when run on a modern smart card\index{smart card}. Hence
...
Card\index{Java Card} and MULTOS\index{MULTOS}, described below.
...
\index{non-volatile memory} (EEPROM\index{EEPROM}), making it available across
sessions. Access to the volatile memory\index{volatile memory} (RAM\index{RAM})
...
\subsubsection{Application Programming Interface}\index{API}
\index{application programming interface|see{API}}
...
cards~\cite{MULTOS_Implementation2010} support a wide range of modulo arithmetic
...
smart card implementation of the Identity Mixer\index{Identity Mixer} technology, which is the first
