...
decisional Diffie-Hellman problem\index{Diffie-Hellman problem!decisional} is easy while the computational Diffie-Hellman\index{Diffie-Hellman problem!computational}
...
Attribute certificates are digital certificates\index{digital cerificate} that bind attributes to users
...
When we consider only a single attribute statement\index{attribute!statement} per public key, the
certificates with the statements\index{attribute!statement} can be stored in a public database and can be
...
Since modular multiplication is not provided by the Java Card\index{Java Card!API} API we used the
NatLib library developed by Hendrik Tews~\cite{TewsJacobs09}. This library
implements modular arithmetic on Java Card\index{Java Card} using basic byte operations as the
...
not a real issue since this verification is only a single point multiplication,\index{point multiplication}
...
the bilinearity of the pairing\index{bilinear pairing} to avoid computing more than two pairings, as
...
In theory it is possible to abuse the RSA cipher\index{RSA!cipher} (and hence use the coprocessor)
...
encrypted using the RSA cipher, which is provided by the Java Card\index{Java Card!API} API.
...
On the one hand, performing a scalar point multiplication\index{point multiplication} (a key agreement
