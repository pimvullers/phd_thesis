...
\subsection{Chaum-Pedersen Signature Scheme\label{sec:CP-scheme}}
...
\end{equation*}
where $s_{ID}$ is the private signing key of the identity\index{identity!provider} provider corresponding
to $P_{ID}$.
...
Luckily the NXP JCOP platform contains some extensions to the standard Java Card\index{Java Card!API}
