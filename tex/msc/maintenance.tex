...
\item Version 1.17, beta release, 2010/12/22

  The handling of the \verb+offset+ argument of the \verb+\mess+,
  \verb+\setstoptimer+, and \verb+\settimeout+ commands is slightly
  modified. All commands with an \verb+offset+ argument can now
  properly handle real numbers as argument.

  The \verb+\gatesymbolradius+ length is now properly implemented and
  documented.

  A new command \verb+\ncondition+ is now available. It has the same
  functionality as the \verb+\condition+ command, but additionally
  puts a cross in the \emph{condition} symbol. Similarly, the
  \verb+\action+ command has a crossed-out counterpart
  \verb+\naction+.

  A bug related to the treatment of the \verb+\topheaddist+,
  \verb+\firstlevelheight+, and \verb+\instheadheight+ lengths is
  resolved. They are not taken into account when computing the height
  of the instance.

  The \verb+\condition*+ macro is now extended to dynamically resize a
  condition symbol that spans multiple instances.

  The \verb+\condition*+ and \verb+\action*+ macros no longer shrink
  to a size smaller than their non-starred counterparts.
