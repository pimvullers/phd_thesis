...
[TODO: mention that this is just basic, and that this scheme is insecure, to
improve security padding schemes as defined in PKCS \#1 should be used.
Also, the same holds for signatures.]
...
[TODO: adjust description to modified MSC protocol]
...
to the randomisation value, whereas the second part commits to the fingerprint
...
[TODO: explain why double mod and why the while loop.]
\begin{algorithm}
...
[TODO: mention that additive notation is common in an ECC setting, whereas multiplicative notation is used for prime order subgroups.]

\subsection{Elliptic Curves}

[TODO: explain notation $\F{q}$ instead of $\Z_q$]
...
\begin{equation*}
  ...
  \end{array}
\end{equation*}
As a result, $e(n\cdot P,\; m\cdot Q) = e(P,\; Q)^{n\,m}$.
...
[TODO: embedding degree is not explained.]

\section{Proofs of Knowledge}

A cryptographic concept that is frequently used in attribute-based credential is
a proof of knowledge. The goal of such proof is for the user, or \emph{prover},
to convince the verifier of a given statement. For example, the often used
challenge-response construction in which a user has to sign or decrypt a
challenge to authenticate herself, is a proof that she knows the private key
corresponding to the public key used by the verifier.
...
can also be constructed for groups in which the order [TODO: of a generator?] is not known to all parties,
which is for instance the case in an RSA setting [TODO: explain.].
As a result the user cannot perform the modular
reduction when computing the response, as . This means that the response no longer hides the
...
[TODO: so the order of $g$ is unknown.]
