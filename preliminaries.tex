...
groups and $\mathbb G_T$ is a multiplicative group and the map is bilinear,
that is, linear in both components. This bilinearity property can be written as
follows:
\begin{equation*}
  \begin{array}{rcl}
...
a proof of knowledge. The goal of such proof is for the user to convince the
verifier of a given statement. For example, the often used challenge-response
construction in which a user has to sign or decrypt a challenge to authenticate
herself, is a proof that she knows the private key corresponding to the public
...
\begin{figure}[ht]
  ...
  \caption{Schnorr's zero-knowledge protocol identification protocol.}
  \label{msc:schnorr}
\end{figure}
...
A similar protocol~\cite{Girault1991} can also be constructed
for groups in which the order is not known to all parties, which is for instance
the case in an RSA setting. As a result the user cannot perform the modular
reduction when computing the response, as depicted in
Figure~\ref{msc:schnorr_mod}. This means that the response no longer hides the
...
knowledge is being proved, can be build by constructing individual proofs for
...
a proof can be build by constructing proofs for $P_1$ and $P_2$ individually,
